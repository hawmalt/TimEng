...
#include <TimEng/TimEng.h>
#include <TimEng/Timing.h>
...
void MainGame::run() {
	...
	TimEng::Music music = m_audioEngine.loadMusic("Sound/XYZ.ogg");
	music.play(-1);
...
void MainGame::initSystems() {
	...
	// Initialize sound, must happen after TimEng::init
	m_audioEngine.init();
	...
	m_spriteFont = new TimEng::SpriteFont("Fonts/chintzy.ttf", 64);
	...
	m_bloodParticleBatch = new TimEng::ParticleBatch2D;
	m_bloodParticleBatch->init(1000, 0.05f, TimEng::ResourceManager::getTexture("Textures/particle.png"), 
		[](TimEng::Particle2D& particle, float deltaTime){
		particle.position += particle.velocity;
		particle.color.a = (GLubyte)(particle.life*255.0);
...
void MainGame::initLevel() {
	...
	m_player->addGun(new Gun("Shotgun", 30, 12, 0.4f, 4, BULLET_SPEED, m_audioEngine.loadSoundEffect("Sound/shots/shotgun.wav")));
	m_player->addGun(new Gun("MP5", 2, 1, 0.2f, 20, BULLET_SPEED, m_audioEngine.loadSoundEffect("Sound/shots/cg1.wav")));
}
...
void MainGame::drawHud() {
	...
	sprintf_s(buffer, "Num Humans %d", m_humans.size()); //copy string into buffer
	m_spriteFont->draw(m_hudSpriteBatch, buffer, glm::vec2(0, 0),
		glm::vec2(0.5), 0.0f, TimEng::ColorRGBA8(255, 255, 255, 255));
...
void MainGame::addBlood(const glm::vec2& position, int numParticles) {
	...
	static std::uniform_real_distribution<float> randAngle(0.0f, 2*3.14159f);

	glm::vec2 vel(2.0f, 0.0f);
	TimEng::ColorRGBA8 col(255, 0, 0, 255);
